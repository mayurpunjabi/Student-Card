<?php
session_start();
$_SESSION['create_card'] = "4";

if(isset($_SESSION['passwd'])){
	$con = mysqli_connect("localhost", "root", "", "studentcard");
	$result = mysqli_query($con, "SELECT ty_placement_activities FROM students WHERE student_id=".$_SESSION['student_id']);
	$count = mysqli_num_rows($result);
	$arr = mysqli_fetch_array($result);
	$result = mysqli_query($con, "SELECT * FROM years WHERE year_id=".$_SESSION['student_id']."3");
	$arry = mysqli_fetch_array($result);
	$result = mysqli_query($con, "SELECT * FROM semester WHERE semester_id=5 AND year_id=".$_SESSION['student_id']."3");
	$arrs1 = mysqli_fetch_array($result);
	$result = mysqli_query($con, "SELECT * FROM semester WHERE semester_id=6 AND year_id=".$_SESSION['student_id']."3");
...
<div class="container" style="box-shadow: 0 4px 10px 4px rgba(19, 35, 47, 0.3);margin-top:0px;">
	<form action="../../assets/submission.php" method="post">
		<div class="row">
			...
			<div class="col-sm-8">
				<div class="form-group" style="margin-top:5px;">
					<h3>Name of your class coordinator:</h3><input class="form-control" style="margin-top:5px;" type = "text"  id="co_name" name = "co_name" placeholder="Enter your Class Coordinator's Name">
				</div>
			</div>
		</div>
		...
		<div class="form-group" style="margin-top:20px;">
			<h3>Have you enrolled yourself for placement activities in our college?</h3>
			<textarea class="form-control" rows = "3" cols = "100" id="placement" name = "placement"></textarea>
		</div>
		...
		<div class="form-group" style="margin-top:20px;">
			<h3>Any suggestions for the improvement of college in terms of its functioning or infrastructure?</h3>
		...
		<div class="form-group" style="margin-top:20px;">
			<TABLE class="table table-sm table-bordered" style="margin-top:20px;">
				<TR> 
					<Th></Th>
					<Th><h3 style="text-align:center;padding:5px;font-weight:400;">Semester V </h3></Th>
					<Th><h3 style="text-align:center;padding:5px;font-weight:400;">Semester VI </h3></Th>
				</TR>
		...
		<div class="prev">
			<a href="../SecondYear" class="btn btn-info" style="width:20%;text-align:center;margin:15px 0PX;">&laquo; Prev</a>
		</div>
		<div class="Next">
			<input type="submit" class="btn btn-success" style="width:20%;text-align:center;margin:15px 0px;" value="Submit &raquo;">
		</div>
		</div>
	</form>
</div>
...
?>
	<script>
		...
		document.getElementById("placement").value = "<?php echo $arr[0];?>";
